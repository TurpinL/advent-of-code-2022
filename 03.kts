...
println("================= Part 1 =================")
val splitRucksacks = rucksacks.map {
    val midPoint = (it.length/2)
    listOf(
        it.subSequence(0 until midPoint),
        it.subSequence(midPoint..it.lastIndex)
    )
}

val wrongItem = splitRucksacks.map {
    it[0].first { item -> it[1].contains(item) }
}

println("Answer:")
wrongItem.sumOf { it.toPriority() }
